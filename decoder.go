...
// NewMSDecoder allocates a new Opus Multistream decoder and initializes it
// with the appropriate parameters. All related memory is managed by the Go GC.
func NewMSDecoder(
	sample_rate int,
	channels int,
	streams int,
	...
	var dec MSDecoder
	err := dec.Init(sample_rate, channels, streams, coupledStreams, mapping)
	if err != nil {
		return nil, err
	}
	return &dec, nil
...
func (dec *MSDecoder) Init(
	...
	size := C.opus_multistream_decoder_get_size(C.int(channels), C.int(coupledStreams))
	dec.sample_rate = sample_rate
	dec.channels = channels
	dec.streams = streams
	dec.coupled_streams = coupledStreams
	dec.mapping = mapping
	dec.mem = make([]byte, size)
	dec.p = (*C.OpusMSDecoder)(unsafe.Pointer(&dec.mem[0]))
	mappingPtr := (*C.uchar)(unsafe.Pointer(&mapping[0]))
	errno := C.opus_multistream_decoder_init(
		dec.p,
		C.opus_int32(sample_rate),
		C.int(channels),
		C.int(streams),
...
// Decode encoded Opus data into the supplied buffer. On success, returns the
// number of samples correctly written to the target buffer.
func (dec *MSDecoder) DecodeFloat32(data []byte, pcm []float32) (int, error) {
	if dec.p == nil {
		return 0, errDecUninitialized
	}
	if len(data) == 0 {
	...
	n := int(C.opus_multistream_decode_float(
		dec.p,
		(*C.uchar)(&data[0]),
		C.opus_int32(len(data)),
		(*C.float)(&pcm[0]),
...
// Note that DecodeFEC automatically falls back to PLC when no FEC data is
// available in the provided packet.
func (dec *MSDecoder) DecodeFEC(data []byte, pcm []int16) error {
	if dec.p == nil {
		return errDecUninitialized
	}
	if len(data) == 0 {
...
// correction. It is to be used on the packet directly following the lost one.
// The supplied buffer needs to be exactly the duration of audio that is missing
func (dec *MSDecoder) DecodeFECFloat32(data []byte, pcm []float32) error {
	if dec.p == nil {
		return errDecUninitialized
	}
	if len(data) == 0 {
...
// PLC does not introduce additional latency. It is calculated from the previous
// packet, not from the next one.
func (dec *MSDecoder) DecodePLC(pcm []int16) error {
	if dec.p == nil {
		return errDecUninitialized
	}
	if len(pcm) == 0 {
...
// DecodePLCFloat32 recovers a lost packet using Opus Packet Loss Concealment feature.
// The supplied buffer needs to be exactly the duration of audio that is missing.
func (dec *MSDecoder) DecodePLCFloat32(pcm []float32) error {
	if dec.p == nil {
		return errDecUninitialized
	}
	if len(pcm) == 0 {
